...
- **Add: authenticated OpenAI-compatible server for private routers** — an opt-in loopback server exposes the bridge model catalog and Chat Completions while every request still runs through Pi's Claude Bridge provider and the official Claude Code Agent SDK harness.
- **Fix: long OpenAI bridge turns no longer time out, retry, or leak workers** — streamed completions now forward inner reasoning and send proxy-safe heartbeat chunks, allow 30 minutes by default, terminate the whole nested Pi process group when the caller disconnects or the deadline expires, and escalate to SIGKILL when graceful shutdown stalls.
